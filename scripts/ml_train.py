...
"""
Script description:
This script implements selected machine learning algorithms to predict
...
def run_experiments() -> None:
    """
    Run machine learning algorithms to predict the survival of Titanic passengers.

    Function loads and preprocesses Titanic dataset, defines ML algorithms that
    will be used for the classification, finds the best hyperparameters for each
    algorithm using GridSearchCV and builds a VotingClassifier using three best
    ML algorithms.
    """
    ...
    models = {
        ...
        "xgboost": XGBClassifier(
            random_state=2025, use_label_encoder=False, eval_metric="logloss"
        ),
    }
    ...
    param_grids = {
        ...
        "naive_bayes": {"var_smoothing": [1e-10, 5e-10, 1e-9, 5e-9, 1e-8]},
        "svm": {
            "C": [0.5, 1.0, 2.5, 5.0],
            "kernel": ["rbf"],
    ...
    best_model_name, best_model_info = max(
        best_models.items(), key=lambda item: item[1]["best_score"]
    )
    ...
    # --- Training & Evaluation of Voting Classifier ---
