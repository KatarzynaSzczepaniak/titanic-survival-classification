...
def extract_title(row: pd.Series) -> pd.Series:
    """
    ...
    Returns:
        Updated row with 'Title', cleaned 'Name' and 'Alias' columns.
    """
...
def extract_deck(row: pd.Series) -> pd.Series:
    """
    ...
    Returns:
        Updated row with 'Deck' and modified 'Cabin'.
    """
...
def full_port_of_embarkation(row: pd.Series) -> pd.Series:
    """
    ...
    Returns:
        Updated row with full port name in 'Embarked'.
    """
...
def get_data(path_to_dataset: str, stage: str) -> pd.DataFrame:
    ...
    Args:
        path_to_dataset: Path to the CSV dataset file.
        stage: One of ['train', 'test']. Determines which columns to return.
    ...
    df = df.apply(extract_deck, axis = 1)
    df = df.apply(full_port_of_embarkation, axis = 1)
    ...
    # Reorder columns
    base_cols = ['Title', 'Name', 'Alias', 'Sex', 'Age', 'Is_child', '# of Siblings/Spouses', '# of Parents/Children', 'Family Size', 'Is_alone',
                 'Ticket', 'Ticket Class', 'Fare', 'Fare per Person', 'Deck', 'Cabin', 'Embarked']
    if stage == 'train':
...
def plot_survivability(df: pd.DataFrame, by: str, bins: int = None, kde: bool = False):
    """
    Plots survivability distribution by a given feature.

    Args:
        df: The Titanic dataset.
        by: Column name to plot survivability against.
        bins (optional): Number of bins (only for numeric data). 
        kde (optional): Whether to include KDE curve (only for numeric data).
    """

    sns.histplot(data = df, x = by, hue = 'Survived',
                 bins = bins, kde = kde, multiple = 'dodge', shrink = 0.8)
